...
2. 先确认目标端：Web 或小程序。
3. 如果用户是要完整复刻某个网站 URL，进入“网站完整克隆模式”，跳过技术栈选择。
4. 非完整克隆模式下，再单独确认技术栈：沿用现有项目，或选择 React、Vue、Next.js、Vite、微信小程序、Taro、uni-app。
5. 只有用户说没有偏好，或现有仓库已经明确技术栈时，才推荐默认技术栈。
6. 分析参考材料：网址、截图、设计稿、PRD、草图、竞品页面或现有仓库。
7. 判断业务功能边界：登录、注册、权限、页面范围、数据来源、表单、状态和异常。
8. 分析页面结构：路由、模块、布局、响应式、交互、点击事件和滚动范围。
9. 只追问无法从材料中可靠判断、且会影响实现的关键信息。
10. 当用户需要设计探索、提供 Open Design 资料，或给了参考网站但只是想模仿风格而非一比一复制时，进入 Open Design 设计阶段。
11. 非完整克隆模式下，生成 `layout-confirmation.html` 低保真布局交互确认稿。
12. 等用户确认页面、跳转、点击事件和滚动范围。
13. 开始高保真实现。
14. 运行项目，截图检查，验证交互和滚动，继续修正。
...
这个模式不再让 Agent 自己随意选择技术栈，而是强制使用：
...
- 所有可观察页面和路由。
- 导航、链接和页面跳转。
- hover、active、focus、selected、disabled 状态。
- 点击效果、弹窗、抽屉、菜单、Tab、手风琴、轮播、筛选、表单、分页。
- 整页滚动、局部滚动、fixed/sticky、横向滚动和响应式断点。
- 颜色、字体、间距、圆角、阴影、图片、视频、图标、动效和媒体资源。
- 视觉 QA、构建检查和项目 README 启动说明。
...
- 项目用途和已实现范围。
- 技术栈。
- 环境要求，例如 Node.js、包管理器、小程序开发者工具等。
- 安装依赖命令。
- 本地开发启动命令。
- 构建命令。
- 预览或生产启动命令。
