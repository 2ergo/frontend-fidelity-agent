...
7. 当用户需要设计探索、提供 Open Design 资料，或给了参考网站但只是想模仿风格而非一比一复制时，进入 Open Design 设计阶段。
8. 生成 `layout-confirmation.html` 低保真布局交互确认稿。
9. 等用户确认页面、跳转、点击事件和滚动范围。
10. 开始高保真实现。
11. 运行项目，截图检查，验证交互和滚动，继续修正。
12. 在生成或大幅修改的前端项目 README 中写明安装、启动、构建、预览、测试和环境配置方式。
