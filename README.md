...
1. 启动阶段一次只问一个问题。
2. 先确认目标是 Web 前端。
3. 如果用户要完整复刻某个网站 URL，进入“网站完整克隆模式”，跳过技术栈选择。
4. 如果用户给的是截图、mockup、Figma 设计、UI 图片或静态设计导出，进入“截图设计稿生成模式”，要求用户指定 React 还是 Vue。
5. 非特殊模式下，再单独确认技术栈：沿用现有项目，或选择 React、Vue、Next.js、Vite。
6. 只有用户说没有偏好，或现有仓库已经明确技术栈时，才推荐默认技术栈。
7. 分析参考材料、业务边界、页面结构、路由、交互、状态、响应式和滚动范围。
8. 只追问无法从材料中可靠判断、且会影响实现的关键信息。
9. 当用户需要设计探索、提供 Open Design 资料，或给了参考网站但只是想模仿风格而非一比一复制时，进入 Open Design 设计阶段。
10. 非完整克隆模式下，必要时生成 `layout-confirmation.html` 低保真布局交互确认稿。
11. 等用户确认页面、跳转、点击事件和滚动范围。
12. 开始实现、运行项目、截图检查、验证交互和滚动，并继续修正。
13. 在生成或大幅修改的前端项目 README 中写明安装、启动、构建、预览、测试和环境配置方式。
